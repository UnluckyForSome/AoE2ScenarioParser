...
"""
Legacy scenario (`genie_scx_py.Scenario`) → DE ``AoE2DEScenario`` mapping.
...
1. **Parse legacy**: `genie_scx_py.Scenario` reads legacy ``.scn`` / ``.scx`` into Python objects.
...
def apply_legacy_scenario_to_de_scenario(scen: Any, scenario: AoE2DEScenario) -> None:
    """
    Apply legacy `genie_scx_py.Scenario` data onto ``scenario`` (see module docstring for philosophy and unmapped
    fields).
    """
